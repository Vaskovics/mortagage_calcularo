...
with open("viktor.txt", "w", encoding="utf-8") as documentation:
    for p in range(period):
        sum_mont_percent = balance * interest
        principal = monthly_payments - sum_mont_percent
        balance = balance - principal
        nb += 1
        print(f"{nb} PaymentЖ {monthly_payments:.2f}£ interest: {sum_mont_percent:.2f}£ principal: {principal:.2f}£ balance: {balance:.2f}£", file=documentation)
